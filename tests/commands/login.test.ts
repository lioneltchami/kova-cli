...
describe("loginCommand", () => {
	...
	it("shows warning when checkSubscription returns null (invalid key)", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn().mockResolvedValue({
				ok: false,
	...
	it("stores credentials even with invalid key (offline fallback)", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn().mockResolvedValue({
				ok: false,
		...
		const creds = readCredentials();
		expect(creds).not.toBeNull();
		expect(creds?.apiKey).toBe("kova_bad_key");
	});
