...
Apple Human Interface Guidelines for iPad apps. 35+ rules across 8 categories covering multitasking, pointer support, sidebar navigation, keyboard shortcuts, Stage Manager, responsive layout, and iPad-specific adaptations. Extends iOS patterns for the larger canvas.
...
_Adaptive layouts, size classes, column-based design, safe areas, all iPad screen sizes. Applies when building any iPad UI, handling regular/compact width transitions, supporting iPad Mini through 13" Pro._
...
_Split View, Slide Over, Stage Manager, resizable windows, multiple scenes. Applies when app runs alongside other apps, user resizes windows, app enters compact width via multitasking._
...
_Sidebar navigation, tab-to-sidebar conversion, three-column layout, toolbar placement, detail views. Applies when designing primary navigation, building information hierarchies, placing toolbars and actions._
...
_Hover effects, pointer magnetism, right-click context menus, scroll behaviors, cursor customization, pointer-driven drag and drop. Applies when any interactive element needs pointer adaptation, supporting Magic Keyboard or trackpad input._
...
_Cmd+key shortcuts, discoverability overlay, tab navigation, hardware keyboard detection, system shortcut conflicts. Applies when adding keyboard shortcuts, building forms, supporting hardware keyboard users._
...
_Scribble handwriting input, double-tap tool switching, pressure/tilt sensitivity, hover detection, PencilKit. Applies when building drawing or note-taking features, supporting handwriting input in text fields._
...
_Inter-app drag and drop, multi-item selection, spring-loaded interactions, Universal Control, drop delegates. Applies when content can be moved between apps, supporting file/image/text transfers, building content organization features._
...
_Extended display content, AirPlay support, display connection/disconnection handling. Applies when app supports presentation mode, external monitors, or AirPlay output._
